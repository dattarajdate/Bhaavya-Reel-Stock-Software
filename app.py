...
def upgrade_db_schema():
    ...
    # Users Table with Granular Permissions Control
    cursor.execute('''
...
# --- MAIN TITLE & SIDEBAR QUICK REEL SEARCH ---
...
if search_reel:
    ...
    if not df_search_rec.empty:
        ...
        df_search_cons = get_data("SELECT c.id as [ID], c.date as [DATE], c.weight_consumed as [USED (KG)], c.machine as [REMARKS / MACHINE] FROM consumption c WHERE c.company_reel = ? ORDER BY c.date ASC", (search_reel,))
        ...
        if not df_search_cons.empty:
            ...
            st.sidebar.dataframe(df_search_cons.drop(columns=['ID']), use_container_width=True)
        else:
            st.sidebar.info("ℹ️ Is reel mein abhi tak koi consumption ya adjustment nahi hua hai.")
    else:
        st.sidebar.error("❌ REEL NO NOT FOUND!")

# Dynamic Permission Checks for Logged User
...
# Tabs Layout Setup based on Roles and Permissions
if st.session_state.user_role == "SUPER_ADMIN":
    ...
    # Custom dynamic tabs for Operators / Viewers based on permissions
    tab_list = []
    if can_view_stk: tab_list.append("📊 LIVE NET STOCK BALANCE")
    tab_list.extend(["📥 GRN RECEIVING ENTRY (MULTIPLE)", "📉 DAILY CONSUMPTION ENTRY", "📈 CONSUMPTION REPORTS", "📋 HISTORY LOGS"])
    
    tabs = st.tabs(tab_list)
    
    # Map tabs dynamically
    idx = 0
    tab_live = tabs[idx] if can_view_stk else None
    if can_view_stk: idx += 1
    tab_rec = tabs[idx]; idx += 1
    tab_cons = tabs[idx]; idx += 1
    tab_rep = tabs[idx]; idx += 1
    tab_hist = tabs[idx]
    tab_adj, tab_msl, tab_import, tab_users = None, None, None, None

# TAB 1: LIVE BALANCE WITH CHART
if 'tab_live' in locals() and tab_live is not None:
    with tab_live:
...
if 'tab_rec' in locals() and tab_rec is not None:
    with tab_rec:
        if not can_add:
...
if 'tab_cons' in locals() and tab_cons is not None:
    with tab_cons:
        if not can_add:
            st.error("🔒 Access Denied: Aapke paas Consumption entry add karne ke rights nahi hain.")
        else:
            st.markdown("<h2 style='text-align: center;'>📉 ADD DAILY CONSUMPTION ENTRY</h2>", unsafe_allow_html=True)
            ...
            if cons_comp_reel:
                ...
                if check_rec.empty: 
                    st.error("❌ BHAAVYA REEL NO NOT FOUND IN STOCK ENTRIES!")
                else:
                    reel_valid = True
                    ...
                    st.info(f"🏋️‍♂️ **Original Weight:** {orig_wt} KG | 📉 **Total Consumed:** {total_consumed} KG | 🔵 **CURRENT BALANCE:** **{current_balance_weight} KG**")
                    if not check_past.empty: 
                        st.dataframe(check_past, use_container_width=True)
            ...
            with st.form("consumption_execution_form", clear_on_submit=True):
                ...
                remaining_weight_str = col2.text_input("ENTER RETURNED / REMAINING WEIGHT ON REEL (KG) *", value="")
                cons_machine = col1.selectbox("MACHINE NAME", ["B FLUTE", "C FLUTE", "5 PLY LINE", "OTHER"])
                
                if st.form_submit_button("SAVE CONSUMPTION ENTRY"):
                    try: 
                        remaining_wt = float(remaining_weight_str) if remaining_weight_str else 0.0
                    except: 
                        remaining_wt = -1.0
                        
                    if not cons_comp_reel: 
                        st.error("❌ Please enter a Bhaavya Reel Number first.")
                    elif not reel_valid: 
                        st.error("❌ Cannot record consumption. Reel invalid or empty.")
                    elif remaining_weight_str == "" or remaining_wt < 0.0: 
                        st.error("❌ Kripya valid Returned/Remaining Weight enter karein.")
                    elif remaining_wt >= current_balance_weight: 
                        st.error(f"❌ Returned weight ({remaining_wt} KG) current balance ({current_balance_weight} KG) se kam hona chahiye!")
                    else:
                        consumed_wt = current_balance_weight - remaining_wt
                        
                        final_machine_remark = cons_machine
                        if remaining_wt <= 200.0:
                            final_machine_remark = f"{cons_machine} - BABY REEL"
                        
                        try:
                            run_query(
                                "INSERT INTO consumption (date, company_reel, weight_consumed, machine) VALUES (?, ?, ?, ?)", 
                                (str(cons_date), cons_comp_reel, float(consumed_wt), final_machine_remark)
                            )
                            st.success(f"🎉 Successfully Saved! Actual Consumed: **{consumed_wt:,.1f} KG**. Remaining Stock: **{remaining_wt:,.1f} KG**")
                            if remaining_wt <= 200.0:
                                st.warning(f"⚠️ Yeh reel 200 KG ya usse kam ({remaining_wt} KG) hai, isliye ispar **'BABY REEL'** remark save kar diya gaya hai!")
                            st.rerun()
                        except Exception as e: 
                            st.error(f"Error: {e}")
...
if tab_adj:
    with tab_adj:
        ...
        if adj_reel_no:
            ...
            if check_rec.empty: st.error("❌ BHAAVYA REEL NO NOT FOUND!")
            else:
...
# TAB 5: MSL & LOW STOCK ALERTS (DYNAMIC INVENTORY DAYS SETUP)
if tab_msl:
    with tab_msl:
        st.markdown("<h2 style='text-align: center;'>🚨 DYNAMIC INVENTORY DAYS & MSL ALERTS</h2>", unsafe_allow_html=True)
        st.info("💡 Yahan aap **Inventory Days** slider se decide karein ki kitne din ka stock maintain karna hai. System automatic per day consumption ke hisab se MSL calculate karega!")
        ...
        inventory_days = st.slider("📅 SELECT TARGET INVENTORY DAYS (MSL Multiplier)", min_value=1, max_value=30, value=10)
        ...
        query_msl_view = f"""
            SELECT 
                ...
                COALESCE(p.per_day_consumption, 0.0) as [PER DAY CONSUMPTION (KG)],
                (COALESCE(p.per_day_consumption, 0.0) * {inventory_days}) as [DYNAMIC MSL (KG)],
        ...
        if not df_msl_check.empty:
            df_msl_check['STATUS'] = df_msl_check.apply(
                lambda row: '🔴 LOW STOCK ALERT' if row['CURRENT STOCK (KG)'] < row['DYNAMIC MSL (KG)'] else '🟢 SUFFICIENT', axis=1
            )
            low_stock_df = df_msl_check[df_msl_check['STATUS'] == '🔴 LOW STOCK ALERT']
            
            if not low_stock_df.empty:
                st.error(f"🚨 CRITICAL ALERT: {len(low_stock_df)} paper specifications have dropped below their {inventory_days}-Day MSL requirement!")
                st.dataframe(low_stock_df, use_container_width=True)
            else:
                st.success(f"✅ All specifications are above their {inventory_days}-Day requirement.")

            st.write("---")
            st.markdown("### ⚙️ UPDATE PER DAY CONSUMPTION & REORDER QTY PER SPECIFICATION")
            ...
            with st.form("update_msl_form"):
                ...
                curr_per_day = float(existing_row['PER DAY CONSUMPTION (KG)'].values[0]) if not existing_row.empty else 0.0
                curr_reorder_val = float(existing_row['REORDER QTY (KG)'].values[0]) if not existing_row.empty else 0.0

                new_per_day_str = col_u5.text_input("PER DAY CONSUMPTION (KG)", value=str(curr_per_day))
                new_reorder_str = st.text_input("NEW REORDER QTY (KG)", value=str(curr_reorder_val))

                if st.form_submit_button("💾 SAVE CONSUMPTION SETTINGS"):
                    try:
                        n_per_day = float(new_per_day_str) if new_per_day_str else 0.0
                        ...
                        """, (int(sel_gsm), int(sel_bf), int(sel_dec), str(sel_shd), n_per_day, n_reorder, n_per_day, n_reorder))
                        st.success("🎉 Per Day Consumption updated successfully!")
                        st.rerun()
            ...
            st.info("ℹ️ No paper stock data found yet.")
...
if 'tab_rep' in locals() and tab_rep is not None:
    with tab_rep:
...
# TAB 8: HISTORY LOGS & EDIT/DELETE CAPABILITY BASED ON SUPER ADMIN RIGHTS
if 'tab_hist' in locals() and tab_hist is not None:
    with tab_hist:
        ...
        with sub_tab1:
            ...
            query_audit_raw = """
                SELECT c.date as [ADJUSTMENT DATE], c.company_reel as [BHAAVYA REEL NO], r.mill as [MILL], r.gsm as [GSM], r.bf as [BF], r.deckle as [DECKLE (MM)], c.weight_consumed as [RAW_WEIGHT], c.machine as [FULL AUDIT REMARKS]
            ...
            if not df_audit_raw.empty:
                ...
                st.dataframe(df_audit_raw[['ADJUSTMENT DATE', 'BHAAVYA REEL NO', 'MILL', 'GSM', 'BF', 'DECKLE (MM)', 'ADJUSTMENT TYPE', 'WEIGHT CHANGE', 'FULL AUDIT REMARKS']], use_container_width=True)
        ...
        with sub_tab2:
            ...
            df_rec_hist = get_data("SELECT r.company_reel as [BHAAVYA REEL NO], r.date as [DATE], r.grn_no as [GRN NO], r.bill_no as [CUSTOMER BILL NO], r.mill as [MILL], r.gsm as [GSM], r.bf as [BF], r.deckle as [DECKLE], r.weight as [WEIGHT], r.rate as [RATE/KG], r.trans_charges as [TRANS CHARGES] FROM receiving r WHERE strftime('%Y-%m-%d', r.date) BETWEEN ? AND ?", (str(st.session_state.r_start), str(st.session_state.r_end)))
            st.dataframe(df_rec_hist, use_container_width=True)
            
            if can_edit_rec:
                st.markdown("#### ✏️ Edit or Delete Receiving Entry")
                edit_bh_reel = st.selectbox("Select Bhaavya Reel No to Edit/Delete", [""] + list(df_rec_hist['BHAAVYA REEL NO'].unique()), key="edit_rec_sel")
                if edit_bh_reel:
                    reel_info = get_data("SELECT date, mill, gsm, bf, deckle, weight, rate, grn_no, bill_no FROM receiving WHERE company_reel = ?", (edit_bh_reel,))
                    if not reel_info.empty:
                        with st.form("edit_receiving_form"):
                            e_date = st.date_input("Date", value=datetime.strptime(reel_info['date'].values[0], "%Y-%m-%d").date())
                            e_wt = st.number_input("Weight (KG)", value=float(reel_info['weight'].values[0]))
                            e_rate = st.number_input("Rate/Kg", value=float(reel_info['rate'].values[0]))
                            e_grn = st.text_input("GRN No", value=str(reel_info['grn_no'].values[0]))
                            e_bill = st.text_input("Bill No", value=str(reel_info['bill_no'].values[0]))
                            
                            col_upd, col_del = st.columns(2)
                            update_clicked = col_upd.form_submit_button("💾 UPDATE RECEIVING")
                            delete_clicked = col_del.form_submit_button("🗑️ DELETE RECEIVING REEL")
                            
                            if update_clicked:
                                run_query("UPDATE receiving SET date = ?, weight = ?, rate = ?, grn_no = ?, bill_no = ? WHERE company_reel = ?", (str(e_date), e_wt, e_rate, e_grn, e_bill, edit_bh_reel))
                                st.success(f"🎉 Reel `{edit_bh_reel}` successfully updated!")
                                st.rerun()
                            if delete_clicked:
                                run_query("DELETE FROM receiving WHERE company_reel = ?", (edit_bh_reel,))
                                run_query("DELETE FROM consumption WHERE company_reel = ?", (edit_bh_reel,))
                                st.success(f"🗑️ Reel `{edit_bh_reel}` and its consumption logs deleted successfully!")
                                st.rerun()
            else:
                st.info("🔒 Note: You do not have permissions to Edit/Delete Receiving entries.")
        ...
        with sub_tab3:
            ...
            query_fix_cons = """
                SELECT c.id as [ID], c.date as [DATE], c.company_reel as [BHAAVYA REEL NO], r.mill as [MILL], r.gsm as [GSM], r.bf as [BF], r.deckle as [DECKLE], c.weight_consumed as [WEIGHT CONSUMED], c.machine as [MACHINE] 
                FROM consumption c LEFT JOIN receiving r ON c.company_reel = r.company_reel 
                ...
                if sel_label:
                    ...
                    if not c_info.empty:
                        with st.form("edit_consumption_form"):
                            ...
                            cupd_clicked = col_cupd.form_submit_button("💾 UPDATE CONSUMPTION")
                            cdel_clicked = col_cdel.form_submit_button("🗑️ DELETE CONSUMPTION ENTRY")
                            
                            if cupd_clicked:
                                run_query("UPDATE consumption SET date = ?, weight_consumed = ?, machine = ? WHERE id = ?", (str(ec_date), ec_wt, ec_mach, sel_cons_id))
                            ...
                            if cdel_clicked:
                                ...
                                st.rerun()
            elif not can_edit_cons:
                st.info("🔒 Note: You do not have permissions to Edit/Delete Consumption entries.")
    ...
    with tab_users:
        ...
        with col_u2:
            ...
            with st.form("add_user_form"):
                ...
                st.markdown("**🛡️ Granular Permissions:**")
                new_perm_view = st.checkbox("Allow Viewing Live Net Stock Balance", value=True)
                new_perm_add = st.checkbox("Allow Adding New Entries (Receiving & Consumption)", value=True)
                new_perm_rec = st.checkbox("Allow Editing/Deleting Receiving Entries", value=False)
                new_perm_cons = st.checkbox("Allow Editing/Deleting Consumption Entries", value=False)
        ...
        st.write("---")
        st.markdown("### ⚙️ MANAGE EXISTING USERS RIGHTS")
        with st.form("manage_rights_form"):
            m_user = st.selectbox("SELECT USER TO UPDATE RIGHTS", user_list, key="m_user_sel")
            curr_row = df_users[df_users['username'] == m_user]
            c_view_val = bool(curr_row['can_view_stock'].values[0]) if not curr_row.empty else True
            c_add_val = bool(curr_row['can_add_entry'].values[0]) if not curr_row.empty else True
            c_rec_val = bool(curr_row['can_edit_receiving'].values[0]) if not curr_row.empty else False
            c_cons_val = bool(curr_row['can_edit_consumption'].values[0]) if not curr_row.empty else False
            
            m_perm_view = st.checkbox("Allow Viewing Live Net Stock Balance", value=c_view_val)
            m_perm_add = st.checkbox("Allow Adding New Entries (Receiving & Consumption)", value=c_add_val)
            m_perm_rec = st.checkbox("Allow Editing/Deleting Receiving Entries", value=c_rec_val)
            m_perm_cons = st.checkbox("Allow Editing/Deleting Consumption Entries", value=c_cons_val)
            
            if st.form_submit_button("💾 SAVE USER RIGHTS"):
