...
if tab_users:
    with tab_users:
        ...
        with st.form("manage_rights_form"):
            m_perm_view = st.checkbox("Allow Viewing Live Net Stock Balance", value=c_view_val, key="m_u_view")
            m_perm_add = st.checkbox("Allow Adding New Entries (Receiving & Consumption)", value=c_add_val, key="m_u_add")
            m_perm_rec = st.checkbox("Allow Editing/Deleting Receiving Entries", value=c_rec_val, key="m_u_rec")
            m_perm_cons = st.checkbox("Allow Editing/Deleting Consumption Entries", value=c_cons_val, key="m_u_cons")
            ...
            if delete_user_clicked:
                if m_user == "super_admin":
                    st.error("❌ Critical Error: Super Admin user ko delete nahi kiya ja sakta!")
                elif m_user == st.session_state.logged_user:
